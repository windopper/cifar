...
class AttentionModuleStage1CIFAR_PreAct(nn.Module):
    """32x32 해상도(또는 사용자 지정 size1) 입력을 위한 CIFAR 전용 Stage-1 Attention (Pre-activation 버전)."""

    def __init__(self, in_channels: int, out_channels: int,
                 size1: Tuple[int, int] = (32, 32), size2: Tuple[int, int] = (16, 16)):
        super().__init__()
        self.first_residual_blocks = PreActivationResidualBlock(in_channels, out_channels)

        self.trunk_branches = nn.Sequential(
            PreActivationResidualBlock(out_channels, out_channels),
            PreActivationResidualBlock(out_channels, out_channels)
        )

        self.mpool1 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.down_residual_blocks1 = PreActivationResidualBlock(out_channels, out_channels)
        self.skip1_connection = PreActivationResidualBlock(out_channels, out_channels)
        ...
        self.interpolation1 = nn.UpsamplingBilinear2d(size=size2)
        self.up_residual_blocks1 = PreActivationResidualBlock(out_channels, out_channels)
        self.interpolation2 = nn.UpsamplingBilinear2d(size=size1)
        ...
        self.last_blocks = PreActivationResidualBlock(out_channels, out_channels)
...
class AttentionModuleStage2CIFAR_PreAct(nn.Module):
    """16x16 해상도 입력을 위한 CIFAR 전용 Stage-2 Attention (Pre-activation 버전)."""
...
class AttentionModuleStage3CIFAR_PreAct(nn.Module):
    """8x8 이하 해상도 입력을 위한 CIFAR 전용 Stage-3 Attention (Pre-activation 버전)."""
