"""
DeepBaselineNetBN3Residual15ConvNeXt: DeepBaselineNetBN3Residual15를 ConvNeXt 블록으로 교체한 버전
...
- Stage 3: 6개의 ConvNeXt block (128 -> 256, 첫 번째는 projection shortcut + stride=2)
- Stage 4: 3개의 ConvNeXt block (256 -> 512, 첫 번째는 projection shortcut + stride=2)
- FC layers: 분류기

기존 DeepBaselineNetBN3Residual15와의 차이점:
- ResidualBlock 대신 ConvNeXtBlock 사용
- Depthwise convolution 커널 크기: 3x3 (기본 7x7에서 변경)
- BatchNorm 대신 LayerNorm 사용
- ReLU 대신 GELU 활성화 함수 사용
...
class DeepBaselineNetBN3Residual15ConvNeXt(nn.Module):
    """
    DeepBaselineNetBN3Residual15를 ConvNeXt 블록으로 교체한 네트워크 (총 15개 ConvNeXt block)
    ...
    4. Stage 3: 6개의 ConvNeXt Block (128 -> 256, 첫 번째는 projection shortcut + stride=2)
    ...
    def __init__(self, init_weights=False):
        super(DeepBaselineNetBN3Residual15ConvNeXt, self).__init__()
        ...
        # ConvNeXt Blocks - 각 stage를 3, 3, 6, 3으로 구성 (총 15개)
    ...
    def forward(self, x):
        ...
        # Stage 3: 6개의 ConvNeXt block (128 -> 256)
