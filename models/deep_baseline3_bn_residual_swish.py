"""
DeepBaselineNetBN3ResidualSwish: DeepBaselineNetBN3Residual에 Swish 활성화 함수를 적용한 버전
...
4. Swish 활성화 함수
   - Swish(x) = x * sigmoid(x) 형태의 부드러운 활성화 함수
   - ReLU보다 더 부드러운 그래디언트를 제공하여 학습 안정성 향상
   - 음수 영역에서도 작은 값을 가지므로 정보 손실 감소

5. 표준 ResNet 구조 적용
   - BasicBlock 스타일의 residual block 사용
   - Conv-BN-Swish 구조를 유지하면서 residual connection 추가
   - 채널 수가 다른 경우 1x1 conv를 사용한 shortcut connection

기존 DeepBaselineNetBN3Residual와의 차이점:
1. 활성화 함수 변경
   - 모든 ReLU를 Swish로 교체
   - ResidualBlock 내부: Conv -> BN -> Swish -> Conv -> BN -> Swish
   - Classifier: Linear -> Swish -> Dropout

2. Residual Block 구조
   - 각 컨볼루션 레이어를 residual block으로 변경
   - 각 block은 두 개의 conv 레이어로 구성 (conv-bn-swish-conv-bn)
...
5. Forward pass 구조 변경
   ...
   - DeepBaselineNetBN3ResidualSwish: Residual Block (F(x) + x 형태)
     * 각 block 내부: Conv -> BN -> Swish -> Conv -> BN
     * Skip connection: identity 또는 projection
     * 최종: Swish(F(x) + x)

참고:
- ResNet 논문: "Deep Residual Learning for Image Recognition" (He et al., 2015)
- Swish 논문: "Searching for Activation Functions" (Ramachandran et al., 2017)
- Residual block은 깊은 네트워크에서 그래디언트 소실 문제를 해결하는 핵심 기술
- Swish는 ReLU의 대안으로 부드러운 활성화를 제공하여 성능 향상 가능
"""
import torch
...
class ResidualBlock(nn.Module):
    """
    ResNet 스타일의 Basic Residual Block (Swish 활성화 함수 사용)
    
    구조:
    - 입력 x
    - Main path: Conv -> BN -> Swish -> Conv -> BN
    - Shortcut: identity (채널/크기 같음) 또는 projection (다름)
    - 출력: Swish(main_path + shortcut)
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        # 첫 번째 Conv-BN-Swish 블록
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=3, 
                               stride=stride, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(out_channels)
        
        # 두 번째 Conv-BN 블록 (Swish는 residual connection 후에 적용)
...
class DeepBaselineNetBN3ResidualSwish(nn.Module):
    """
    DeepBaselineNetBN3Residual에 Swish 활성화 함수를 적용한 네트워크
    
    구조:
    1. 초기 Conv-BN-Swish (3 -> 64)
    ...
    def __init__(self, init_weights=False):
        super(DeepBaselineNetBN3ResidualSwish, self).__init__()
    ...
    def _initialize_weights(self):
        """가중치 초기화 - Swish는 ReLU와 유사하므로 Kaiming initialization 사용"""
