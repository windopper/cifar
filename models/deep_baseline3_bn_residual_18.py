"""
DeepBaselineNetBN3Residual20: DeepBaselineNetBN3Residual15의 레이어 개수와 채널을 늘린 버전
...
네트워크 구조:
- conv1: 초기 feature extraction (3 -> 128)
- Stage 1: 4개의 residual block (128 -> 128, identity shortcut)
- Stage 2: 4개의 residual block (128 -> 256, 첫 번째는 projection shortcut + stride=2)
- Stage 3: 8개의 residual block (256 -> 512, 첫 번째는 projection shortcut + stride=2)
- Stage 4: 4개의 residual block (512 -> 1024, 첫 번째는 projection shortcut + stride=2)
- FC layers: 분류기

기존 DeepBaselineNetBN3Residual15와의 차이점:
- 초기 채널 수 증가: 64 -> 128
- Stage 1: 2개 -> 4개 블록, 채널 64 -> 128
- Stage 2: 2개 -> 4개 블록, 채널 128 -> 256
- Stage 3: 4개 -> 8개 블록, 채널 256 -> 512
- Stage 4: 2개 -> 4개 블록, 채널 512 -> 1024
- 총 residual block 수: 10개 -> 20개
- 최종 채널 수: 512 -> 1024
...
class DeepBaselineNetBN3Residual18(nn.Module):
    """
    DeepBaselineNetBN3Residual15의 레이어 개수와 채널을 늘린 네트워크 (총 20개 residual block)

    구조:
    1. 초기 Conv-BN-ReLU (3 -> 128)
    2. Stage 1: 4개의 Residual Block (128 -> 128, identity shortcut)
    3. Stage 2: 4개의 Residual Block (128 -> 256, 첫 번째는 projection shortcut + stride=2)
    4. Stage 3: 8개의 Residual Block (256 -> 512, 첫 번째는 projection shortcut + stride=2)
    5. Stage 4: 4개의 Residual Block (512 -> 1024, 첫 번째는 projection shortcut + stride=2)
    ...
    def __init__(self, init_weights=False):
        ...
        # 입력: 3채널 (RGB), 출력: 128채널 (기존 64에서 증가)
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(64)

        # Residual Blocks - 각 stage를 4, 4, 8, 4로 구성 (총 20개)
        # Stage 1: 4개의 block (64 -> 64, identity shortcut)
        self.stage1 = _make_layer(64, 64, num_blocks=3, stride=1)

        # Stage 2: 4개의 block (128 -> 256, 첫 번째는 projection shortcut + stride=2)
        self.stage2 = _make_layer(64, 128, num_blocks=3, stride=2)

        # Stage 3: 8개의 block (256 -> 512, 첫 번째는 projection shortcut + stride=2)
        self.stage3 = _make_layer(128, 256, num_blocks=9, stride=2)

        # Stage 4: 4개의 block (512 -> 1024, 첫 번째는 projection shortcut + stride=2)
    ...
    def forward(self, x):
        ...
        # Stage 1: 4개의 residual block (128 -> 128)
        # 32x32 -> 32x32
        x = self.stage1(x)

        # Stage 2: 4개의 residual block (128 -> 256)
        # 32x32 -> 16x16 (stride=2로 다운샘플링)
        x = self.stage2(x)

        # Stage 3: 8개의 residual block (256 -> 512)
        # 16x16 -> 8x8 (stride=2로 다운샘플링)
        x = self.stage3(x)

        # Stage 4: 4개의 residual block (512 -> 1024)
        # 8x8 -> 4x4 (stride=2로 다운샘플링)
        x = self.stage4(x)

        # Global Average Pooling: [batch, 1024, 4, 4] -> [batch, 1024, 1, 1]
