...
def get_criterion(name: str, label_smoothing: float = 0.0):
    ...
    if name.lower() not in criterions:
        raise ValueError(f"Unknown criterion: {name}. Available: {list(criterions.keys())}")
...
def get_scheduler(name: str, optimizer, epochs: int = 24, steps_per_epoch: int = 1, 
    ...
    if name.lower() == 'exponentiallr':
        schedulers['exponentiallr'] = lr_scheduler.ExponentialLR(optimizer, gamma=gamma)
...
def parse_args():
    ...
    parser.add_argument('--batch-size', type=int, default=16, help='배치 크기 (default: 16)')
    ...
    parser.add_argument('--epochs', type=int, default=24, help='에포크 수 (default: 24)')
    parser.add_argument('--save-prefix', type=str, default='', 
                        help='모델 저장 파일명 접두사 (default: baseline)')
    parser.add_argument('--lr', type=float, default=0.001, help='학습률 (default: 0.001)')
    parser.add_argument('--momentum', type=float, default=0.9, help='모멘텀 (default: 0.9)')
...
def main():
    ...
    net = net.to(device)
    criterion = get_criterion(args.criterion, label_smoothing=args.label_smoothing)
    optimizer = get_optimizer(args.optimizer, net, lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    ...
    for epoch in range(args.epochs):
        ...
        for i, data in enumerate(pbar, 0):
            ...
            pbar.set_postfix({'loss': f'{running_loss / (i + 1):.3f}', 'lr': f'{current_lr:.6f}'})
